    ...
    def __init__(self, config_path="config1.ini"):
        ...
        self.defaults = {
            ...
            "PARAMETERS": {
                ...
                "max_context_tokens": "6000", # This is more for context window, less directly for SambaNova's max_tokens param
                "max_output_tokens_sambanova": "1024", # Max tokens for SambaNova response
...
if __name__ == "__main__":
    ...
    st.title("💬 SatSure Proposal Q&A System (SambaNova)")
    st.caption("Query Project Proposals & Supporting Data with SambaNova LLM (v10-SambaNova)")
    ...
    def load_rag_system(config_path="config1.ini"):
        ...
        system = None
        with st.spinner("Initializing RAG System (SambaNova) & loading models..."):
            try:
                system = RAGSystem(config_path=config_path) # RAGSystem now handles its own key check message
            except Exception as e:
                logger.error(f"Fatal RAG Initialization Error (SambaNova): {e}", exc_info=True)
        ...
        st.sidebar.success("✅ RAG System (SambaNova) Initialized.")
        return system
    ...
    try:
        ...
        config_details = {
            "LLM (SambaNova)": rag_sys.config.sambanova_model, # UPDATED
        ...
        if prompt := st.chat_input("Ask a question about the SatSure proposals..."):
            ...
            with st.spinner("🛰️ SatSure AI (SambaNova) is thinking..."):
